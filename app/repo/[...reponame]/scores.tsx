...
export default async function Scores({ reponame }: { reponame: any[] }){
    ...
    const scoreMap = pulls.reduce((score: Map<string, { count: number; avatar: string }>, pull: any) => {
    ...
    return (
        <>
            {scoreArray.map((value: { name: string; value: { count: number; avatar: string } }, index: number) => (
